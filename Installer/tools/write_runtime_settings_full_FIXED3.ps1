...
function Ensure-Obj([psobject]$parent, [string]$name) {
  ...
  if ($prop -and $null -ne $prop.Value -and ($prop.Value -is [psobject])) {
    return $prop.Value
  }
...
if ([string]::IsNullOrWhiteSpace($Cam2Path)) { $Cam2Path = if ($Cam2Protocol -eq 'axis_http_mjpeg') { '/axis-cgi/mjpg/video.cgi' } else { '/axis-media/media.amp' } }
if ([string]::IsNullOrWhiteSpace($Cam3Path)) { $Cam3Path = if ($Cam3Protocol -eq 'axis_http_mjpeg') { '/axis-cgi/mjpg/video.cgi' } else { '/axis-media/media.amp' } }
...
Set-Prop $cam2 'Enabled' (To-Bool $Cam2Enabled $true)
...
Set-Prop $cam3 'Enabled' (To-Bool $Cam3Enabled $true)
...
if (Test-Path $pdSettings) {
  try {
    ...
    $backup = Join-Path $pdDir ("settings.runtime.json.disabled_by_installer_{0}" -f $stamp)
    Move-Item -Force $pdSettings $backup
    Write-Host ("ProgramData runtime settings moved aside: {0}" -f $backup)
  }
  catch {
    Write-Warning ("Could not move aside ProgramData settings file: {0}" -f $_.Exception.Message)
  }
}

$ActivationCode | Set-Content -Encoding UTF8 -Force (Join-Path $docDir 'install_code.txt')
$ActivationCode | Set-Content -Encoding UTF8 -Force (Join-Path $pdDir 'install_code.txt')

Write-Host 'OK: full runtime settings saved to Documents.'
Write-Host ("Documents:   {0}" -f $docSettings)
